 ...
 */
public class MAChangeAttributeNamesReplace extends AbstractDataProcessing {

	public static final String PARAMETER_SEARCH_ATTRIBUTE_NAME = "search";
	public static final String PARAMETER_REPLACE_ATTRIBUTE_NAME = "replacement";
	...
	public MAChangeAttributeNamesReplace(OperatorDescription description) {
		super(description);
	}
	...
	public ExampleSet apply(ExampleSet exampleSet) throws OperatorException {
		ExampleSet ex2 = exampleSetInput2.getData(ExampleSet.class);
...
//		String replaceByString = isParameterSet(PARAMETER_REPLACE_BY) ? getParameterAsString(PARAMETER_REPLACE_BY) : "";
		String searchAttributeName = getParameterAsString(PARAMETER_SEARCH_ATTRIBUTE_NAME);
		String replaceAttributeName = getParameterAsString(PARAMETER_REPLACE_ATTRIBUTE_NAME);
		
		try {
			for (Attribute attribute : attributeSubset) {
				
				for (Example ex : ex2) {
					if (ex.getNominalValue(ex.getAttributes().get(searchAttributeName)).equals(attribute.getName())) {
						String replacement = ex.getNominalValue(ex.getAttributes().get(replaceAttributeName));
						System.out.println(attribute.getName() + " gefunden und durch " + replacement + " ersetzt");
						
						attribute.setName(replacement);
					}
...
//			throw new UserError(this, 152, replaceByString);
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(new ParameterTypeAttribute(PARAMETER_SEARCH_ATTRIBUTE_NAME, "The name of the nominal attribute to which values should be added.", exampleSetInput2, false));
		types.add(new ParameterTypeAttribute(PARAMETER_REPLACE_ATTRIBUTE_NAME, "The name of the nominal attribute to which values should be added.", exampleSetInput2, false));
	...
	public ResourceConsumptionEstimator getResourceConsumptionEstimator() {
		return OperatorResourceConsumptionHandler.getResourceConsumptionEstimator(getInputPort(), MAChangeAttributeNamesReplace.class, attributeSelector);
	}
}
