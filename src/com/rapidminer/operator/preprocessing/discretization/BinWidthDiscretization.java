 ...
 */
public class BinWidthDiscretization extends AbstractDiscretizationOperator {

	static {
		registerDiscretizationOperator(BinWidthDiscretization.class);
	}
	...
	public BinWidthDiscretization(OperatorDescription description) {
		super(description);
	}
...
//		}
		...
		for (Attribute attribute : exampleSet.getAttributes()) {
			if (attribute.isNumerical()) {
				...
				int thresholdSize = (int) (max/binWidth+0.5);  
				double[] thresholds = new double[thresholdSize];
				
//				ArrayList<Double> thresholds = new ArrayList<Double>();
//				TreeSet<Tupel<Double, Double>> thresholdPairs = new TreeSet<Tupel<Double, Double>>();
				
				for (int i = 0; i < thresholdSize; i++) {
					thresholds[i] = (i+1)*binWidth; // start with first threshold at 1*binWidth
				}
				
				ranges.put(attribute, thresholds);
			}
		}

		DiscretizationModel model = new DiscretizationModel(exampleSet);
		model.setRanges(ranges, "range", DiscretizationModel.RANGE_NAME_SHORT, -1);
		return model;
	}
	...
	public ResourceConsumptionEstimator getResourceConsumptionEstimator() {
		return OperatorResourceConsumptionHandler.getResourceConsumptionEstimator(getInputPort(), BinWidthDiscretization.class, attributeSelector);
	}
}
