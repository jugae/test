...
public class FastKMeansSubseq extends RMAbstractClusterer implements CapabilityProvider{
	...
	public FastKMeansSubseq(OperatorDescription description) {
		super(description);
...
//				for (int bitter=0; bitter<exampleSet.size(); bitter++) {
//					Attribute attribute = exampleSet.getExample(bitter).getAttributes().get("cluster");
//					int clusterAssignment = (int)exampleSet.getExample(bitter).getNumericalValue(attribute);
//					model.assignExample(clusterAssignment, getAsDoubleArray(exampleSet.getExample(bitter), attributes));
//				}
			}
			model.finishAssign();
